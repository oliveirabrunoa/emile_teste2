...
class CourseType(Base):
    ...
    ##Relationships##
    course_types = relationship('Courses',back_populates='course_type')
...
class Institution(Base):
    ...
    ##Relationships##
    institute = relationship('Program',back_populates='institution')
...
class Users(Base):
    ...
    teachers = relationship('CourseSections',back_populates='teacher')
...
class CourseSections(Base):
    ...
    course = relationship('Courses',back_populates='course_sections',lazy='joined')
    
    teacher_id = Column('course_sections_teacher_id',Integer,ForeignKey('IFUSERS.users_id'))
    teacher = relationship('Users',back_populates='teachers',lazy='joined')
...
class Courses(Base):
    ...
    ##Relationships##
    course_sections = relationship('CourseSections',back_populates='course')
    
    course_type_id = Column('courses_course_type_id',Integer,ForeignKey('IFCOURSETYPE.course_type_id'))
    course_type = relationship('CourseType',back_populates='course_types',lazy='joined')
    
    program_id = Column('courses_program_id',Integer,ForeignKey('IFPROGRAM.program_id'))
    program = relationship('Program',back_populates='programs',lazy='joined')
...
class Program(Base):
    ...
    programs = relationship('Courses',back_populates='program')
    
    institution_id = Column('program_institution_id',Integer,ForeignKey('IFINSTITUTION.institution_id'))
    institution = relationship('Institution',back_populates='institute',lazy='joined')
